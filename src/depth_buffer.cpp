...
{
	depth_buffer.reserve(static_cast<size_t>(
		width * height));
}
...
void cg::ZCulling::Clear()
{
	...
	depth_buffer.clear();
	depth_buffer.resize(
		static_cast<size_t>(width * height), 1.0f);
}
...
void cg::ZCulling::DrawTriangle(face face)
{
	float x_min = std::min(
		std::min(
			face.vertexes[0].x, face.vertexes[1].x),
		face.vertexes[2].x);
	float x_max = std::max(
		std::max(
			face.vertexes[0].x, face.vertexes[1].x), 
	...
	float y_min = std::min(
		std::min(
			face.vertexes[0].y, face.vertexes[1].y), 
		face.vertexes[2].y);
	float y_max = std::max(
		std::max(
			face.vertexes[0].y, face.vertexes[1].y), 
	...
	float area = direction * EdgeFunction(
		face.vertexes[0].xy(), 
		face.vertexes[1].xy(), 
		face.vertexes[2].xy());
	...
	for (float x = x_min; x <= x_max; x += 1.0f) {
		for (float y = y_min; y <= y_max; y += 1.0f) {
			float2 point{ x, y };
			float e0 = direction * EdgeFunction(
				face.vertexes[0].xy(), 
				face.vertexes[1].xy(), 
				point);
			float e1 = direction * EdgeFunction(
				face.vertexes[1].xy(), 
				face.vertexes[2].xy(), 
				point);
			float e2 = direction * EdgeFunction(
				face.vertexes[2].xy(), 
				face.vertexes[0].xy(),
				point);
