...
void cg::Projections::Rasterizer(face face)
{
	...
	for (unsigned i = 0; i < 3; i++) {
		...
		face.vertexes[i].x = std::clamp(
			x_center - scale * face.vertexes[i].x, 0.f, width - 1.f);
		face.vertexes[i].y = std::clamp(
			y_center - scale * face.vertexes[i].y, 0.f, height - 1.f);
	}
	DrawTriangle(face);
}
